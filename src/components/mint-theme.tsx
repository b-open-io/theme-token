...
export function MintTheme({ className = "" }: MintThemeProps) {
	...
	const [activeTab, setActiveTab] = useState<"presets" | "css" | "json">(
		"presets",
	);
			...
			{!isConnected && status !== "connecting" && (
				<div className="rounded-lg border border-amber-500/50 bg-amber-500/10 p-4">
					<p className="text-sm text-amber-700 dark:text-amber-300">
						Connect your wallet using the button in the header to inscribe theme
						tokens.
					</p>
				</div>
			)}
  ...
  /* ... */
					...
					{customCss && !validationError && parseMetadata && (
						<div className="flex items-center gap-2 text-sm text-green-600">
							...
							Theme imported successfully - {parseMetadata.totalPropertyCount}{" "}
							properties
			...
			{/* Custom Label */}
			<div>
				<label htmlFor="theme-name" className="mb-2 block text-sm font-medium">
					Theme Name (optional)
				</label>
				<input
					id="theme-name"
