...
interface AudioFrequencyAnalyser {
...
export function AudioVisualizer3D() {
	...
	const analyserRef = useRef<AudioFrequencyAnalyser | null>(null);
	const uniformsRef = useRef<VisualizerUniforms | null>(null);
	const animationIdRef = useRef<number | null>(null);

	const _isPlaying = useAudioStore((s) => s.isPlaying);
	...
	useEffect(() => {
		...
		if (audioElement) {
			try {
				...
				const themeContainer =
					containerRef.current.closest('[style*="--primary"]') ||
		...
		const animate = () => {
			...
			if (analyserRef.current && uniformsRef.current) {
				...
				const textureData = uniformsRef.current.tAudioData.value.image?.data;
				if (data && textureData) {
