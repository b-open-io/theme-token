...
export function AudioVisualizerSimple() {
	...
	useEffect(() => {
		...
		const getThemeColors = () => {
			const container =
				canvas.closest('[style*="--primary"]') || document.documentElement;
			...
			const backgroundColors = ["--background", "--card", "--muted"];
			...
			if (waveColors.length < 3) {
				const primary =
					formatColor(style.getPropertyValue("--primary").trim()) ||
					"hsl(220 90% 56%)";
				const accent =
					formatColor(style.getPropertyValue("--accent").trim()) ||
					"hsl(280 80% 60%)";
		...
		const draw = () => {
			...
			if (!analyserRef.current || !dataArrayRef.current) {
				...
				for (let i = 0; i < 50; i++) {
					...
					const y =
						canvas.height / 2 + Math.sin(i * 0.2 + Date.now() * 0.001) * 20;
					if (i === 0) {
						ctx.moveTo(x, y);
					} else {
						ctx.lineTo(x, y);
			...
			const bassIntensity = Math.min(
				1,
				Array.from(freqData.slice(0, 8)).reduce((a, b) => a + b, 0) / (8 * 200),
			);
			...
			for (let band = 0; band < waveCount; band++) {
				...
				const verticalOffset =
					Math.sin(Date.now() * 0.001 + band) * bandIntensity * 20;
				...
				for (let i = 0; i < bufferLength; i += step) {
					...
					const freqModulation =
						1 + (freqData[Math.min(i, freqData.length - 1)] / 255) * 0.5;
	...
	return (
		<div
			ref={containerRef}
			className="rounded-xl border shadow-sm h-full relative overflow-hidden"
		>
			<canvas
				ref={canvasRef}
