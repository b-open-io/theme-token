	...
	useEffect(() => {
		...
		const parse = (str: string, fallback: THREE.Vector3) => {
			...
			if (cleanStr.includes("oklch")) {
				try {
					const match =
						cleanStr.match(/oklch\(([\d.]+)\s+([\d.]+)\s+([\d.]+)\)/) ||
						cleanStr.match(
					...
					if (match) {
						...
						const gamma = (c: number) =>
							c <= 0.0031308
								? 12.92 * c
								: 1.055 * Math.abs(c) ** (1.0 / 2.4) - 0.055;
			...
			try {
				const s = cleanStr.replace(/hsl\(|\)|%|deg|,|\//g, " ");
				const parts = s
					.split(" ")
					.filter((x) => x && x.trim() !== "")
					.map(parseFloat);
		...
		const _c4 = parse(style["--chart-4"] || style["chart-4"], a);
		const _c5 = parse(style["--chart-5"] || style["chart-5"], p);
