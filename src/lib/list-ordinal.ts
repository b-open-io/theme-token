...
import { P2PKH, Script, Transaction, UnlockingScript, Utils } from "@bsv/sdk";
import { OrdLock } from "js-1sat-ord";
import type { Ordinal, SignatureRequest, YoursWallet } from "./yours-wallet";
...
export async function listOrdinal(
	...
	console.log(
		"[listOrdinal] Starting with ordinal:",
		ordinal,
		"price:",
		priceSatoshis,
	);
	...
	console.log(
		"[listOrdinal] Selected payment UTXO:",
		JSON.stringify(payUtxo, null, 2),
	);
	...
	console.log(
		"[listOrdinal] Fee estimate - OrdLock script:",
		ordLockScriptSize,
		"bytes, total tx:",
		estimatedSize,
		"bytes",
	...
	const ordinalLockingScript = Script.fromBinary(
		Utils.toArray(ordinal.script, "base64"),
	);
	const payUtxoLockingScript = Script.fromBinary(
		Utils.toArray(payUtxo.script, "hex"),
	);

	console.log(
		"[listOrdinal] Ordinal locking script hex:",
		ordinalLockingScript.toHex(),
	);
	console.log(
		"[listOrdinal] PayUtxo locking script hex:",
		payUtxoLockingScript.toHex(),
	);
	...
	for (const sig of signatures) {
		console.log(
			"[listOrdinal] Building unlocking script for input",
			sig.inputIndex,
		);
