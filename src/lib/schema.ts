...
export const themeStylePropsSchema = z.object({
  ...
  // Dimensions (matching tweakcn internal format)
  radius: z.string(),
  "letter-spacing": z.string().optional(), // tweakcn uses this internally, CSS outputs as --tracking-normal
  spacing: z.string().optional(),

  // Shadow components (matching tweakcn internal format)
  // tweakcn uses shadow-offset-x/y internally, CSS outputs as --shadow-x/y
  ...
  // Shadow presets (generated by tweakcn from shadow components)
 ...
 *
 * Maps CSS variable names to tweakcn internal format:
...
function parseCssBlock(css: string): ThemeStyleProps {
  ...
  while ((match = varRegex.test(css), match = varRegex.exec(css)) !== null) {
    ...
    // Map CSS names to tweakcn internal JSON format
    if (name === "shadow-x") {
